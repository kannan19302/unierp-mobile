...
import '../../data/datasources/saas_remote_data_source.dart';
import '../../data/repositories/saas_repository_impl.dart';
import '../../domain/entities/saas.dart';
import '../../domain/repositories/saas_repository.dart';
import '../../domain/usecases/saas_usecases.dart';

final Provider<SaasRemoteDataSource> saasRemoteDataSourceProvider =
    Provider<SaasRemoteDataSource>(
  (Ref ref) => SaasRemoteDataSourceImpl(ref.watch(apiClientProvider)),
);

final Provider<SaasRepository> saasRepositoryProvider =
    Provider<SaasRepository>(
  (Ref ref) => SaasRepositoryImpl(
    remote: ref.watch(saasRemoteDataSourceProvider),
...
final NotifierProvider<SaasPlanListController, SaasPlanListState>
    saasPlanListControllerProvider =
    NotifierProvider<SaasPlanListController, SaasPlanListState>(
  SaasPlanListController.new,
);

class SaasPlanListController extends Notifier<SaasPlanListState> {
  Timer? _searchDebounce;

  @override
  SaasPlanListState build() {
    ref.watch(activeTenantIdProvider);
    ref.onDispose(() => _searchDebounce?.cancel());
    Future<void>.microtask(refresh);
    return const SaasPlanListState();
  }

  ListSaasPlansUseCase get _listUseCase =>
      ListSaasPlansUseCase(ref.read(saasRepositoryProvider));
...
class SaasSubscriptionListState extends Equatable {
  ...
  final List<SaasSubscription> items;
  final PaginationMeta meta;
  final ListQuery query;
  final bool isLoading;
  final bool isLoadingMore;
...
class SaasSubscriptionListController extends Notifier<SaasSubscriptionListState> {
  ...
  @override
  SaasSubscriptionListState build() {
    ref.watch(activeTenantIdProvider);
    ref.onDispose(() => _searchDebounce?.cancel());
    Future<void>.microtask(refresh);
    return const SaasSubscriptionListState();
  }

  ListSaasSubscriptionsUseCase get _listUseCase =>
      ListSaasSubscriptionsUseCase(ref.read(saasRepositoryProvider));
...
final NotifierProvider<SaasInvoiceListController, SaasInvoiceListState>
    saasInvoiceListControllerProvider =
    NotifierProvider<SaasInvoiceListController, SaasInvoiceListState>(
  SaasInvoiceListController.new,
);

class SaasInvoiceListController extends Notifier<SaasInvoiceListState> {
  @override
  SaasInvoiceListState build() {
    ref.watch(activeTenantIdProvider);
    Future<void>.microtask(refresh);
    return const SaasInvoiceListState();
  }

  ListSaasInvoicesUseCase get _listUseCase =>
      ListSaasInvoicesUseCase(ref.read(saasRepositoryProvider));
...
final NotifierProvider<SaasTenantListController, SaasTenantListState>
    saasTenantListControllerProvider =
    NotifierProvider<SaasTenantListController, SaasTenantListState>(
  SaasTenantListController.new,
);

class SaasTenantListController extends Notifier<SaasTenantListState> {
  Timer? _searchDebounce;

  @override
  SaasTenantListState build() {
    ref.watch(activeTenantIdProvider);
    ref.onDispose(() => _searchDebounce?.cancel());
    Future<void>.microtask(refresh);
    return const SaasTenantListState();
  }

  ListSaasTenantsUseCase get _listUseCase =>
      ListSaasTenantsUseCase(ref.read(saasRepositoryProvider));
